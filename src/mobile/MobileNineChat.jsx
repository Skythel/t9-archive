  ...
  return (
    <>
      <Box className={classes.nineChatHeader}>
        <Box style={{ marginLeft: "auto" }}><img src={IconNINE} alt="NINE Icon" height={32} style={{ verticalAlign: "middle" }} /></Box>
        <Box>Message</Box>
        <Box style={{ marginLeft: "auto", paddingRight: "5px" }}>
          <Link to={`/`}>
            <IconButton>
              <img src={IconClose} alt="Close NINE" height={28} className={classes.chatBack} />
            </IconButton>
          </Link>
        </Box>
      </Box>
        ...
        {chatList.map((c, i) => (
          <Link to={`/chat/${i + 1}`} className={classes.nineChatListLink}>
            <Box className={classes.nineChatListItem}>
              <Box>
                <img src={IconCasual} alt={c.title === "" ? "Group Chat" : c.title} height={50} className={classes.groupIcon} />
              </Box>
              <Box textAlign="left">
